...
function filterVisibleStaffNotifs(rows,role){
  const live=(rows||[]).filter(x=>!isPastMeetingNotif(x));
  if(role!=="super_admin")return live;
  return live.filter(x=>x.type==="staff_message");
}

// Module-level so parent badge updates don't remount this page (lost open/expand state).
function NotificationsPage({user,isAdmin,isMobile,toast,setNotifBadge,setSelClient,setPage}){
