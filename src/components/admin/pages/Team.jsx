...
import { actIcon, isBdmRole, isAgentRole, staffRoleLabel } from "../../../lib/helpers";
    ...
    if(teamView){
      ...
      return(<div>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(3,1fr)",gap:14,marginBottom:20}}>
          ...
          {(isBdmRole(m.role)||isAgentRole(m.role))&&<StatCard label="Clients assigned" value={assigned.length} icon="👥" color={T.blue} soft={T.blueSoft}/>}
          <StatCard label="Last active" value={memberActs[0]?.date?memberActs[0].date.split(" at ")[0]:"–"} icon="🕐" color={T.violet} soft={T.violetSoft}/>
        </div>
        {(isBdmRole(m.role)||isAgentRole(m.role))&&<Card style={{marginBottom:16}}>
          <SectionTitle sub={isAgentRole(m.role)?"Clients this Agent works on (backend)":"Clients this BDM owns (chat / calls)"}>Assigned clients</SectionTitle>
          ...
          {canManageAssign&&<Btn variant="ghost" size="sm" style={{marginTop:12}} onClick={()=>setModal({type:"assign",agent:m})}>Manage assignments</Btn>}
        </Card>}
        <Card>
          ...
          {memberActs.length===0?<Empty icon="📭" title="No activity yet" sub="This member's actions will appear here."/>:
            memberActs.map((a,i)=>(<div key={a.id} className="hoverRow" style={{display:"flex",gap:12,padding:"11px 8px",borderBottom:i<memberActs.length-1?`1px solid ${T.line}`:"none",alignItems:"flex-start"}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{a.desc}</div><div style={{fontSize:11,color:T.faint,marginTop:2}}>{a.date}{a.clientId&&a.clientId!=="__internal"?` · ${allClients.find(c=>c.id===a.clientId)?.businessName||""}`:""}</div></div>
            </div>))}
        </Card>
      </div>);
    }
    return(<div>
      <PageHead isMobile={isMobile} title="Team" sub={`${visibleStaff.length} team members`} right={isStaffMgr&&<Btn onClick={()=>setModal({type:"team"})}>+ Invite Member</Btn>}/>
      {visibleStaff.map((m)=>(<Card key={m.id} hover style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
          ...
          <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
            {(isBdmRole(m.role)||isAgentRole(m.role))&&<span style={{fontSize:11,color:T.sub,fontWeight:700,background:T.blueSoft,padding:"3px 9px",borderRadius:20}}>{allClients.filter(c=>c[isAgentRole(m.role)?"assignedAgentId":"assignedBdmId"]===m.id).length} clients</span>}
