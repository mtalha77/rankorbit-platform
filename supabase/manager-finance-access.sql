-- Manager Finance access (run once in Supabase SQL Editor). Safe to re-run.
-- Super Admin → Team → Permissions → "Can view Finance & monthly revenue"

alter table profiles add column if not exists "canViewFinance" boolean default false;

-- Clients cannot self-grant this field (staff / service_role still can update).
